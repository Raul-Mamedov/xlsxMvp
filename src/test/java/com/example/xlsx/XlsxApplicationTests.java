...
class XlsxApplicationTests {
	...
	public void shouldListAllFiles() throws Exception {
		...
		this.mvc.perform(get("/")).andExpect(status().isOk())
				.andExpect(model().attribute("files",
						Matchers.contains("http://localhost/files/first.xlsx",
								"http://localhost/files/second.xlsx")));
	}

	@Test
	public void shouldSaveUploadedFile() throws Exception {
		MockMultipartFile multipartFile = new MockMultipartFile("file", "test.xlsx",
				"xlsx/plain", "Spring Framework".getBytes());
	...
	public void should404WhenMissingFile() throws Exception {
		given(this.storageService.loadAsResource("test.xlsx"))
